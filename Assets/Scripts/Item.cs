...
public class Item : MonoBehaviour, IDraggable
{
	...
	private List<Shelf> _touchingShelves = new List<Shelf>();
	private int _originIndex;
	private ItemLayer _originLayer;
	...
	{
		return transform.DOLocalMove(position, duration).SetEase(Ease.InBack)
			.OnComplete(() => Renderer.sortingOrder = 0)
			.WaitForCompletion();
	}

	private Shelf GetClosetShelf()
	{
